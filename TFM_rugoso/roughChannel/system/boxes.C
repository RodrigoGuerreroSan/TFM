...
#include <fstream>
#include <random>
#include <cmath> // Cambié math.h por cmath para usar funciones matemáticas en C++
using namespace std;

int main()
{
    const double d50 = 0.01; // mean grain diameter
    const double c = 3; // scaling factor
    const double x = 4; // streamwise domain size
    const double z = 2; // spanwise domain size

    // Ajusta estos valores para cambiar la altura de los cuadros
    const double mean = 0; // mean
    const double sigma = 0.5 * d50; // standard deviation
    const double height_scale = 2.0; // factor de escalado adicional para las alturas
    ...
    for (int i = 0; i < nx; i++)
    {
        for (int j = 0; j < nz; j++)
        {
            // Multiplicamos por height_scale para ajustar las alturas
            number[i][j] = distribution(generator) * height_scale;
        }
    }
    ...
    for (int i = 0; i < nx; i++)
    {
        for (int j = 0; j < nz; j++)
        {
            myfile << "(" << X[i] << " 0 " << Z[j] << ")(" << X[i + 1] << " " << number[i][j] << " " << Z[j + 1] << ")\n";
        }
    }
    myfile.close();
    return 0;
